...
> **Hinweis zur Begrifflichkeit:** Die App-Oberfläche zeigt heute **"Track"** statt "Fach" und **"Wiki"** statt "Nachschlag" — das sind reine Anzeigenamen. Die Format-Schlüsselwörter in der `.md`-Datei (`# Fach:`, `# Nachschlag`) sind davon unabhängig und bleiben **exakt so geschrieben**, unabhängig davon, wie die App die Begriffe im UI nennt.

## 1. Grundprinzip

Eine `.md`-Datei entspricht **einem Track** (in der Datei technisch "Fach" genannt). Sie besteht aus bis zu vier Blöcken, die als H1-Überschriften (`#`) beginnen:

```markdown
# Fach: <Name des Tracks>
...
- **Fehlt `Typ:` komplett, nimmt der Parser stillschweigend `radio` an** — das führt zu keinem Fehler, kann aber eine unbeabsichtigt falsche Aufgabenart erzeugen. Gib `Typ:` deshalb **immer explizit** an, auch bei Radio-Aufgaben.
...
```
`O:` = eine Option pro Zeile. **Genau eine** Zeile trägt ein `*` direkt vor dem Text — das ist die korrekte Antwort. Sind es **null oder mehr als eine** markierte Optionen, bricht der Import mit Fehler ab.
...
```
Wie `radio`, aber **mindestens eine** Option trägt `*`; mehrere sind erlaubt und üblich.
...
```
`L:` ist **genau eine** Zeile mit dem Wert `true` oder `false` (englisch, klein geschrieben) — jeder andere Wert bricht den Import ab.
...
- **Freitext-Lücke:** `{{Lösung}}` — mehrere akzeptierte Schreibweisen mit `;` trennen: `{{Zielvariable;Label;Target}}`. Der Abgleich mit der Nutzereingabe ist **tolerant**: Groß-/Kleinschreibung wird ignoriert, führende/abschließende Leerzeichen entfernt, mehrfache Leerzeichen zusammengefasst. Du musst also **keine** Groß-/Kleinschreib-Varianten oder Leerzeichen-Varianten separat auflisten.
- **Auswahl-Lücke (Dropdown):** Optionen mit `|` trennen, korrekte mit `*` markieren: `{{Hoth|*Dagobah|Endor}}`. Genau **ein** `*` pro Auswahl-Lücke ist Pflicht.
...
- `K: <Kartentext> -> <Zielspalte>` — die Zielspalte muss **exakt** einer `S:`-Zeile entsprechen (Text muss übereinstimmen, wird getrimmt — Leerzeichen direkt um `->` spielen also keine Rolle).
...
## 4. Block `# Nachschlag` — Referenz-/Wiki-Einträge
...
- In der App-Oberfläche heißt dieser Bereich "Wiki" — das Format-Schlüsselwort bleibt trotzdem `# Nachschlag`.
...
- **Feldpräfixe** (`F:`, `A:`, `H1:`–`H5:`, `Typ:`, `Titel:`, `O:`, `L:`, `S:`, `K:`) stehen immer am Zeilenanfang und bleiben **immer in dieser Form** — auch in Dateien mit anderssprachigem Inhalt (siehe Abschnitt 9).
...
- Bei `radio`/`checkbox`-Optionen markiert ein `*` **direkt vor dem Options-Text** die korrekte Antwort. Soll eine Options-Text selbst mit einem Sternchen beginnen, ist das aktuell nicht sauber unterscheidbar — in der Praxis kommt das kaum vor, aber im Zweifel den Optionstext so umformulieren, dass er nicht mit `*` beginnt.

### Typische Fehler, die den Import abbrechen lassen
- `# Fach:` fehlt oder ist nicht die erste Zeile
- `###` (Aufgabe) außerhalb eines `##`-Kapitels
- `radio`/`truefalse` ohne **genau eine** korrekte Antwort markiert (weder keine noch mehrere)
...
- [ ] Jede `###`-Aufgabe hat `Typ:` **explizit gesetzt** (nicht weggelassen), `F:`, und mindestens eine gültige Antwortmarkierung für ihren Typ
- [ ] Jede `radio`/`truefalse`-Aufgabe hat **genau eine** korrekte Antwort (nicht null, nicht mehrere)
...
## 9. Andere Inhaltssprache (z. B. englische Lerninhalte)

Der Inhalt (Fragen, Antworten, Kapitel-/Kartentitel, Wiki-Text) kann in jeder Sprache verfasst werden. Die **Struktur-Schlüsselwörter bleiben davon komplett unberührt** — sie sind Format-Syntax, keine übersetzbaren Textbausteine:

`# Fach:`, `# Test`, `# Karten`, `# Nachschlag`, `### Aufgabe`, `Typ:`, `Titel:`, `F:`, `A:`, `O:`, `L:`, `S:`, `K:`, `H1:`–`H5:`, sowie die Typ-Werte `radio`, `checkbox`, `truefalse`, `fillblank`, `block` und die Wert `true`/`false` bei `L:`.

Eine englischsprachige Datei sieht also z. B. so aus:
...
Nur `World War II`, `Key Events`, `Major turning points...`, die Frage und die Optionen sind Englisch — `# Fach:`, `# Test`, `### Aufgabe`, `Typ:`, `radio`, `F:`, `O:`, `A:` bleiben exakt wie hier gezeigt.
